...
class ConfigMethodActivity : FragmentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val methods = resources.getStringArray(R.array.array_methods)

        for (method in methods) {
            menuItems.add(MenuItem(android.R.drawable.ic_menu_compass, "", method))
        }
        ...
        recyclerView.adapter = MenuAdapter(this, R.layout.notitle_item, menuItems, object: MenuAdapter.AdapterCallback {
            override fun onItemClicked(menuPosition: Int?) {
                ...
                sharedPreferences.edit().putInt("METHOD", menuPosition!!).apply()
                finish()
            }
        })
